  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-16 text-center font-fontRaleway">
        <DropdownMenuItem className="pl-3 hover:bg-primary cursor-pointer" onClick={() => changeLanguage("tr")}>
          <div className="flex space-x-2 items-center">
            <Images url={turkeyFlag} addClass=" w-6 h-6 " />
            <p className="font-medium">TR</p>
          </div>
        </DropdownMenuItem>
        <DropdownMenuItem className="pl-3 hover:bg-primary cursor-pointer" onClick={() => changeLanguage("en")}>
        <div className="flex space-x-2 items-center">
...

{
  /* <Button
      ...
      className="shadow-xl border-primary text-primary hover:text-white bg-secondary border text-[16px] rounded-[15px] px-3 py-5 transition duration-300 ease-in-out"
    >
      {isEnglish ? "Tr" : "En"}{" "}
    </Button> */
}
